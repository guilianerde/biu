...
const LyricsModal = ({ isOpen, onOpenChange }: LyricsModalProps) => {
  ...
  const defaultTitle = useMemo(() => playItem?.pageTitle || playItem?.title || "", [playItem?.pageTitle, playItem?.title]);
  const defaultArtist = useMemo(() => playItem?.ownerName || "", [playItem?.ownerName]);
  ...
  useEffect(() => {
    if (!isOpen) return;
    setTitleKeyword(current => (current.trim() ? current : defaultTitle));
    setArtistKeyword(current => (current.trim() ? current : defaultArtist));
  }, [defaultArtist, defaultTitle, isOpen]);

  const buildQuery = () => {
    const title = titleKeyword.trim();
    const artist = artistKeyword.trim();
    if (!title) return "";
        ...
        <ModalBody className="gap-4">
          <div className="flex flex-col gap-3">
            <div className="grid gap-3 md:grid-cols-[minmax(0,2fr)_minmax(0,1fr)]">
              ...
              <Input
                value={artistKeyword}
                onValueChange={setArtistKeyword}
                placeholder="可选：歌手名"
                onKeyDown={event => {
                  if (event.key === "Enter") {
                    handleSearch();
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {(defaultTitle || defaultArtist) && (
                <Button
                  variant="light"
                  onPress={() => {
                    setTitleKeyword(defaultTitle);
                    setArtistKeyword(defaultArtist);
                  }}
                >
                  使用当前播放
                </Button>
